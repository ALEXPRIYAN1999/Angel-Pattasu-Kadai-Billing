...
DEFAULT_SETTINGS = {
    ...
    "address":    "AYWANAR NAGAR, NH-7, PATTAMPUDHUR",
...
def _bill_totals(bill):
    """Return the invoice totals used consistently by GUI, PDF and saved bills."""
    total_orig = 0.0
    total_discounted = 0.0
    for it in bill.get("items", []) or []:
        qty = float(it.get("qty", 0) or 0)
        orig = float(it.get("orig_rate", it.get("rate", 0)) or 0)
        rate = float(it.get("rate", 0) or 0)
        total_orig += qty * orig
        total_discounted += qty * rate
    total_discount = total_orig - total_discounted
    packing = max(0.0, float(bill.get("packing_charge", 0) or 0))
    before_round = total_discounted + packing
...
def generate_a4_invoice(bill, out_path):
    ...
    pay_status = str(bill.get('payment_status', 'Paid') or 'Paid')
    paid_amt = float(bill.get('amount_paid', totals['grand_total']) or 0)
    balance_amt = max(0.0, totals['grand_total'] - paid_amt)
    ...
    c.drawString(20*mm, grand_y-14*mm,
...
class AngelApp(tk.Tk):
    ...
    def show_dashboard(self):
        ...
        today_total = sum(b.get("grand_total", 0) for b in today_bills)
        all_total   = sum(b.get("grand_total", 0) for b in BILLS)
    ...
    def show_billing(self, edit_bill=None):
        ...
        if edit_bill is not None:
            ...
            self.payment_status.set(edit_bill.get("payment_status", "Paid") or "Paid")
            self.amount_paid.set(f"{float(edit_bill.get('amount_paid', edit_bill.get('grand_total', 0)) or 0):.2f}")
    ...
    def _billing_left(self, parent):
        ...
        tk.Label(
            parent,
            text="Enter = add / manual item  •  ↑↓ browse  •  Double-click = add",
    ...
    def filter_products(self):
        ...
        if q:
            ...
            self.filtered_products = active[:200]
    ...
    def _billing_right(self, parent):
        ...
        self.cust_address.grid(row=1, column=1, columnspan=3,
        ...
        self.payment_combo = ttk.Combobox(
            pay, textvariable=self.payment_status, values=["Paid", "Not Paid"],
    ...
    def _current_cart_totals(self):
        data = {"items": self.cart, "packing_charge": self._current_packing_charge()}
    ...
    def _apply_payment_status(self):
        ...
        total = self._current_cart_totals()["grand_total"]
        if status == "Paid":
            ...
            try:
                ...
                current = 0.0
            self.amount_paid.set(f"{max(0.0, min(float(total), current)):.2f}")
    ...
    def _start_cart_edit(self, idx, field):
        ...
        entry.bind(
            "<Return>",
            lambda e: self._commit_cart_edit(move_after=(field == "Qty"))
        )
        entry.bind(
            "<Tab>",
            lambda e: self._commit_cart_edit(move_after=(field == "Qty"))
        ...
        entry.bind(
            "<FocusOut>",
            lambda e: self.after_idle(self._commit_cart_edit)
        )
    ...
    def save_bill(self):
        ...
        calc = _bill_totals({"items": self.cart, "packing_charge": packing})
        sub = calc["sub_total"]
        gt = calc["grand_total"]
        rnd = calc["roundoff"]
        ...
        if status == "Paid":
            ...
            try:
                paid = max(0.0, min(float(gt), float(self.amount_paid.get() or 0)))
            except (TypeError, ValueError):
                paid = 0.0
    ...
    def _bill_payment_values(self, bill):
        total = float(bill.get("grand_total", 0) or 0)
        status = str(bill.get("payment_status", "Paid") or "Paid")
        if status == "Paid":
            paid = total
        else:
            try:
        ...
        for b in reversed(BILLS):
            ...
            self.hist_tree.insert(
                ...
                values=(
                    ...
                    len(b.get("items", [])), f"₹ {b.get('grand_total', 0):,.2f}",
                    status, f"₹ {paid:,.2f}", f"₹ {balance:,.2f}"
                )
            )
            shown.append(b)

        total_value = sum(float(b.get("grand_total", 0) or 0) for b in shown)
